...
## doplnena funkce dle pozadovaneho typu
      ...
      stddoors  = Hash.new ##
      stdwindows  = Hash.new ##
      stdceilingsurfaces  = Hash.new ##
      stdinteriorwallsurfaces  = Hash.new ##
      stdfloorsurfaces = Hash.new ##
      stdnosurfacetype = ""
      stdpolygonpos = 0
      ...
      roofsurfaces = Hash.new
	    floorsurfaces = Hash.new ##
      intbuildinginstallations  = Hash.new ##
      buildinginstallations  = Hash.new ##
      doors  = Hash.new ##
      windows  = Hash.new ##
      ceilingsurfaces  = Hash.new ##
        ...
        if(entity.class ==Sketchup::Group or entity.class == Sketchup::ComponentInstance)
          ...
          explodedentities.each do |explode|
            if(explode.class == Sketchup::Face)
              ...
              polygonpos = writegroup(explode.entities(), transformation * explode.transformation, 1, tw, actbuildingid, polygonpos, groundsurfaces, roofsurfaces, wallsurfaces, floorsurfaces, outerfloorsurfaces, intbuildinginstallations, buildinginstallations, doors, windows, ceilingsurfaces, interiorwallsurfaces, nosurfacetype)
            elsif(explode.class == Sketchup::ComponentInstance)
              if(explode.name != "")
                puts "exporting #{explode.name}"
              end ## doplneny pozadovany typ
              polygonpos = writegroup(explode.definition.entities, transformation * explode.transformation, 1, tw, actbuildingid, polygonpos, groundsurfaces, roofsurfaces, wallsurfaces, floorsurfaces, outerfloorsurfaces, intbuildinginstallations, buildinginstallations, doors, windows, ceilingsurfaces, interiorwallsurfaces, nosurfacetype)
            end
          end
          #Alle Entities sind in Buffern gespeichert und können in XML-Datei
          #gespeichert werden ##doplneny pozadovany typ
          if(groundsurfaces.size > 0 or wallsurfaces.size > 0 or roofsurfaces.size > 0 or floorsurfaces.size > 0 or outerfloorsurfaces.size > 0  or intbuildinginstallations.size > 0 or buildinginstallations.size > 0 or doors.size > 0 or windows.size > 0 or ceilingsurfaces.size > 0 or interiorwallsurfaces.size > 0 or nosurfacetype != "")
            ...
            writebuffers(@lod, groundsurfaces, wallsurfaces, roofsurfaces, floorsurfaces, outerfloorsurfaces, intbuildinginstallations, buildinginstallations, doors, windows, ceilingsurfaces, interiorwallsurfaces, nosurfacetype, actbuildingid)
            nosurfacetype = ""
            @handle << "</bldg:Building>\n"
            @handle << "</core:cityObjectMember>\n"
          end
          ...
          stdnosurfacetype << writeface(entity,Geom::Transformation.new, 1, tw, stdbuildingid, stdpolygonpos, stdgroundsurfaces, stdroofsurfaces, stdwallsurfaces, stdfloorsurfaces, stdouterfloorsurfaces, stdintbuildinginstallations, stdbuildinginstallations, stddoors, stdwindows, stdceilingsurfaces, stdinteriorwallsurfaces, stdnosurfacetype)
        end
      ...
      if(stdbuilding != "")
        ...
        writebuffers(@lod, stdgroundsurfaces, stdwallsurfaces, stdroofsurfaces, stdfloorsurfaces, stdouterfloorsurfaces, stdintbuildinginstallations, stdbuildinginstallations, stddoors, stdwindows, stdceilingsurfaces, stdinteriorwallsurfaces, stdnosurfacetype, stdbuildingid)
        @handle << "</bldg:Building>\n"
        @handle << "</core:cityObjectMember>\n"
      end
    ...
    def writebuffers(lod, groundsurfaces, wallsurfaces, roofsurfaces, floorsurfaces, outerfloorsurfaces, intbuildinginstallations, buildinginstallations, doors, windows, ceilingsurfaces, interiorwallsurfaces, nosurfacetype, actbuildingid)## doplneny pozadovany typ
...
if(interiorwallsurfaces.size > 0)
    ...
    def writegroup(entities, transformation, za, tw, actbuildingid, polygonpos, groundsurfaces, roofsurfaces, wallsurfaces, floorsurfaces, outerfloorsurfaces, intbuildinginstallations, buildinginstallations, doors, windows, ceilingsurfaces, interiorwallsurfaces, nosurfacetype)
      if(za > @maxtiefe)
        @maxtiefe += 1
      end
      entities.each do |entity|
        if(entity.class == Sketchup::Face)
          ...
          polygonpos = writegroup(entity.entities, transformation *  entity.transformation, za + 1, tw, actbuildingid, polygonpos, groundsurfaces, roofsurfaces, wallsurfaces, floorsurfaces, outerfloorsurfaces, intbuildinginstallations, buildinginstallations, doors, windows, ceilingsurfaces, interiorwallsurfaces, nosurfacetype)
          ...
          polygonpos = writegroup(entity.definition.entities, transformation * entity.transformation , za + 1, tw, actbuildingid, polygonpos, groundsurfaces, roofsurfaces, wallsurfaces, floorsurfaces, outerfloorsurfaces, intbuildinginstallations, buildinginstallations, doors, windows, ceilingsurfaces, interiorwallsurfaces, nosurfacetype)
        end
      end
      return polygonpos
    end
    ...
    def writeface(face, transformation, za, tw, actbuildingid, polygonpos, groundsurfaces, roofsurfaces, wallsurfaces, floorsurfaces, outerfloorsurfaces, intbuildinginstallations, buildinginstallations, doors, windows, ceilingsurfaces, interiorwallsurfaces, nosurfacetype)
      ...
      when 3
	    ...
	    when 5
        if(floorsurfaces[surfaceid] == nil)
          floorsurfaces[surfaceid] = []
        end
        floorsurfaces[surfaceid] << tmpfile
        
      when 7
        if(outerfloorsurfaces[surfaceid] == nil )
          outerfloorsurfaces[surfaceid] = []
        end
        outerfloorsurfaces[surfaceid] << tmpfile
      when 6
        if(buildinginstallations[surfaceid] == nil)
          buildinginstallations[surfaceid] = []
        end
        buildinginstallations[surfaceid] << tmpfile
      when 8
        if(intbuildinginstallations[surfaceid] == nil)
          intbuildinginstallations[surfaceid] = []
        end
        intbuildinginstallations[surfaceid] << tmpfile
      when 10
        if(ceilingsurfaces[surfaceid] == nil)
          ceilingsurfaces[surfaceid] = []
        end
        ceilingsurfaces[surfaceid] << tmpfile
      when 11
       if(interiorwallsurfaces[surfaceid] == nil)
          interiorwallsurfaces[surfaceid] = []
       end
        interiorwallsurfaces[surfaceid] << tmpfile
      when 4
        if(doors[surfaceid] == nil)
          doors[surfaceid] = []
        end
        doors[surfaceid] << tmpfile       
      when 9
        if(windows[surfaceid] == nil)
          windows[surfaceid] = []
        end
        windows[surfaceid] << tmpfile
